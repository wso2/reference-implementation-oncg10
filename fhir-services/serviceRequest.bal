...
# public type ServiceRequest r4:ServiceRequest;
public type ServiceRequest uscore700:USCoreServiceRequestProfile;

# A service representing a network-accessible API
service /fhir/r4/ServiceRequest on new fhirr4:Listener(config = serviceRequestApiConfig) {
    // Read the current state of single resource based on its id.
    isolated resource function get [string id](r4:FHIRContext fhirContext) returns ServiceRequest|r4:OperationOutcome|r4:FHIRError|error {
        anydata|r4:OperationOutcome|r4:FHIRError|error result;
        lock {
            result = fetchResourceById(fhirContext, "ServiceRequest", id, uscore700:USCoreServiceRequestProfile);
        }
        if result is ServiceRequest {
            return result;
        }
    ...
    isolated resource function get .(r4:FHIRContext fhirContext) returns r4:Bundle|r4:OperationOutcome|r4:FHIRError|error {
        ...
        lock {
            searchResult = searchResourceBundle(fhirContext, "ServiceRequest");
        }
        return searchResult;
    }
