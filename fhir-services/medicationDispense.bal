...
public type MedicationDispense uscore700:USCoreMedicationDispenseProfile;

# A service representing a network-accessible API
service /fhir/r4/MedicationDispense on new fhirr4:Listener(config = medicationDispenseApiConfig) {

    // Read the current state of single resource based on its id.
    isolated resource function get [string id](r4:FHIRContext fhirContext) returns MedicationDispense|r4:OperationOutcome|r4:FHIRError|error {
        anydata|r4:OperationOutcome|r4:FHIRError|error result;
        lock {
            result = fetchResourceById(fhirContext, "MedicationDispense", id, uscore700:USCoreMedicationDispenseProfile);
        }
        if result is MedicationDispense {
            return result;
        }
    ...
    isolated resource function get .(r4:FHIRContext fhirContext) returns r4:Bundle|r4:OperationOutcome|r4:FHIRError|error {
        ...
        lock {
            searchResult = searchResourceBundle(fhirContext, "MedicationDispense");
        }
        return searchResult;
    }
